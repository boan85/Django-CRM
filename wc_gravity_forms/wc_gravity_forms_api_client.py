...
class Client(object):
    ...
    def build_url(self, method, route, query_args=None, expires=None):
        ...
        try:
            ...
            private_key = bytearray(self.private_key, 'utf-8')
            string_to_sign = bytearray(string_to_sign, 'utf-8')
        ...
        if query_args:
            copy_query_args = query_args.copy()
            copy_query_args.update(required_args)
            query = urllib.parse.urlencode(
                copy_query_args
                # dict(query_args.items() + required_args.items())
            )
        else:
            query = urllib.parse.urlencode(required_args)
    ...
    def get_forms_list(self):
        ...
        while good_response:
            try:
                form = self.get_form(counter)
                return_list.append(form)
                counter += 1
